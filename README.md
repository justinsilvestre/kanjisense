...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create kanjisense
  fly apps create kanjisense-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kanjisense
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kanjisense-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'kanjisense-staging' does not match app name in config file 'kanjisense'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name kanjisense-db
  fly postgres attach --app kanjisense kanjisense-db

  fly postgres create --name kanjisense-staging-db
  fly postgres attach --app kanjisense-staging kanjisense-staging-db
  ```
