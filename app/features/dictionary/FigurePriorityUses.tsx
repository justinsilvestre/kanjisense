...
export function FigurePriorityUses({
  ...
  return (
    <section className={`${className} flex flex-wrap flex-row gap-4 p-8 `}>
      <h2 className=" text-gray-600 basis-full">used as a component in:</h2>
      <ul className="basis-full flex flex-row flex-wrap gap-4 justify-between">
        {priorityUses.concat(fetcherData?.firstClassUses || []).map((u) => {
          ...
          return (
            <li
              key={u.parentId}
              className={`group [flex-basis:5rem] align-top ${
        ...
        {!fetcherData?.firstClassUses && count > PRELOADED_USES_COUNT ? (
          <li
            className="[flex-basis:5rem] flex-grow pb-10 [height:5rem]"
          ...
          <li className="[flex-basis:5rem] flex-grow"> </li>
        )}
      </ul>
    </section>
  );
