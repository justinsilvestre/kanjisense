...
export async function seedKanjisenseSoundMarks(
  prisma: PrismaClient,
  force = false,
) {
  const seeded = await prisma.readyTables.findUnique({
    where: { id: "KanjisenseSoundMark" },
  });
  if (seeded && !force) console.log(`KanjisenseSoundMark already seeded. 🌱`);
    ...
    for (const { id, etymology: etymologyText } of kanjiDbEtymologies) {
      ...
      if (originCharacter) {
        ...
        dbInput.set(id, {
          character: id,
          soundMark: originCharacter.char,
          chain,
        });
      }
    }
    ...
    await prisma.kanjisenseSoundMark.createMany({
      data: [...dbInput.values()].map(({ character, soundMark, chain }) => ({
        character,
        activeSoundMark: soundMark,
        chain: { set: chain.map((item) => item.toJSON()) },
      })),
    });
...
interface CreateSoundMarkInput {
  character: string;
  soundMark: string;
  chain: CharacterOriginReference[];
}
